...
@MODELS.register_module()
class BAN_MLPDecoder_DS(BaseModule): 
    ## 添加深度监督机制
    def __init__(self,
                 in_channels,
                 channels,
    ...
    def cls_seg(self, feat, in_channels):

        conv_seg = nn.Conv2d(in_channels, self.out_channels, kernel_size=1)
        conv_seg = conv_seg.to(feat.device)
        """Classify each pixel."""
        if self.dropout is not None:
            feat = self.dropout(feat)
        feat = feat.float() 
    ...
    def forward(self, inputs1, inputs2):
        # 用于存储输出的 list，第一个是原始的融合结果，其余是每个尺度的预测结果
        out_list = []
        ...
        out = self.discriminator(out)
        out = self.cls_seg(out, out.shape[1])

        out_list.append(out)

        # 深度监督：遍历每一层特征图，单独进行处理
        for idx in range(len(inputs1)):
            ...
            # 根据拼接后的通道数动态创建 discriminator
            discriminator = MixFFN(
                embed_dims=out.shape[1],  # 根据拼接后的通道数动态设置
                feedforward_channels=out.shape[1],  # 设置为拼接后的通道数
                ffn_drop=0.,
                dropout_layer=dict(type='DropPath', drop_prob=0.),
                act_cfg=dict(type='GELU')
            )
            # 将 discriminator 转移到 out 所在的设备
            discriminator = discriminator.to(out.device)
