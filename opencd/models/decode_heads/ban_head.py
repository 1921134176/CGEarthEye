...
class BitemporalAdapterBranch(nn.Module):
    ...
    Args:
        clip_channels (int): Number of channels of visual features.
            Default: 768.
        fusion_index (List[int]): The layer number of the encode transformer to fuse with the CLIP feature.
            Default: [0, 1, 2].
...
class BitemporalAdapterHead(BaseDecodeHead):
    ...
    def predict(self, inputs: Tuple[torch.Tensor], batch_img_metas: List[dict],
        ...
        if isinstance(mask_props, list):
            return self.predict_by_feat(mask_props[0], batch_img_metas)
        else:
            return self.predict_by_feat(mask_props, batch_img_metas)
    ...
    def loss(self, x: Tuple[torch.Tensor], batch_data_samples: SampleList,
        ...
        ## 代码更改2024/12/04 
        # 判断 seg_logits 是否为 list 类型
        if isinstance(seg_logits, list):
            total_loss = {}
            # 遍历 seg_logits 列表，计算每一项的损失
            for logit in seg_logits:
                loss_item = self.loss_by_feat(logit, batch_data_samples)
                # 对每个 loss_item 进行处理，按 key 分别累加损失
                for key, value in loss_item.items():
                    if key not in total_loss:
                        total_loss[key] = value  # 如果 key 不在字典中，初始化
                    else:
                        total_loss[key] += value  # 如果 key 已存在，累加值
            return total_loss
        else:
            # 如果 seg_logits 不是 list 类型，直接计算损失
            losses = self.loss_by_feat(seg_logits, batch_data_samples)
            return losses