...
@TRANSFORMS.register_module()
class MultiImgRandomResample(BaseTransform):
    """Randomly resample image resolution while maintaining original dimensions.

    This transform simulates varying spatial resolutions by:
    1. Downscaling the image to a lower resolution (scale_factor < 1)
    2. Upscaling back to original size with interpolation

    Added Keys:
    - resample_scale (float | None): Applied scale factor
    ...
    Args:
        prob (float): Probability of applying resampling (0-1)
        min_scale (float): Minimum scale factor (e.g., 0.5)
        max_scale (float): Maximum scale factor (e.g., 2.0)
        img_interpolation (str): Image interpolation method, default 'bilinear'
        seg_interpolation (str): Segmentation map interpolation, default 'nearest'
    """
    ...
    def __init__(self,
                 ...
                 min_scale: float = 0.5,
                 max_scale: float = 1.5,
                 img_interpolation: str = 'bilinear',
                 seg_interpolation: str = 'nearest'):

        assert 0 <= prob <= 1, f"Invalid probability {prob}"
        assert 0 < min_scale <= max_scale, f"Invalid scale range [{min_scale}, {max_scale}]"

        self.prob = prob
        self.scale_range = (min_scale, max_scale)
        self.img_interp = img_interpolation
        self.seg_interp = seg_interpolation

    @cache_randomness
    def _get_resample_params(self):
        """Randomly determine resampling parameters with caching"""
        if np.random.rand() > self.prob:
            return None  # No resampling

        # Log-uniform sampling for better perceptual linearity
        scale = np.exp(np.random.uniform(
            np.log(self.scale_range[0]),
            np.log(self.scale_range[1])
        ))
        return scale

    def _resample_image(self, img: np.ndarray, scale: float) -> np.ndarray:
        """Resample single image with scale factor"""
        h, w = img.shape[:2]
        temp_size = (int(w * scale), int(h * scale))  # (width, height)

        # Downscale-upscale pipeline
        downscaled = mmcv.imresize(img, temp_size, interpolation=self.img_interp)
        upscaled = mmcv.imresize(downscaled, (w, h), interpolation=self.img_interp)
        return upscaled

    def transform(self, results: dict) -> dict:
        """Apply resolution resampling transformation"""
        scale = self._get_resample_params()

        if scale is None:
            results['resample_scale'] = None
            return results

        # Process multi-band images
        results['img'] = [self._resample_image(img, scale) for img in results['img']]
        ...
        for key in results.get('seg_fields', []):
            ...
            downscaled = mmcv.imresize(seg, temp_size, interpolation=self.seg_interp)
            upscaled = mmcv.imresize(downscaled, (w, h), interpolation=self.seg_interp)
            results[key] = upscaled.astype(seg.dtype)

        results['resample_scale'] = scale
        return results

    def __repr__(self):
        return (f'{self.__class__.__name__}(prob={self.prob}, '
                f'scale_range={self.scale_range}, '
                f'img_interp={self.img_interp}, '
                f'seg_interp={self.seg_interp})')
